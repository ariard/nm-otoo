...
/*   Updated: 2018/02/12 20:17:26 by ariard           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
				...
				char *ptr, t_data *data)
{
	...
	while (segsects--)
	{
		if (!ft_strcmp("__text", sect->sectname))
			ft_hexdump32(sect, ptr, data, "Contents of (__TEXT,__text) section");
		if (!ft_strcmp("__data", sect->sectname) && (data->flag &
			OT_DOPT))
			ft_hexdump32(sect, ptr, data, "Contents of (__DATA,__data) section");
